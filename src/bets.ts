import { BetParams, Bet, BetResult } from './types.js';
 ...
 */
export function validateOutcome(outcome: string, field: string = 'outcome'): void {
  const normalizedOutcome = outcome.toUpperCase();
  if (normalizedOutcome !== 'YES' && normalizedOutcome !== 'NO') {
    throw new ValidationError(
      `${field} must be 'YES' or 'NO' (case-insensitive)`,
      field
    );
  }
}
 ...
 */
export function validateAmount(amount: number, field: string = 'amount'): void {
 ...
 */
export function validateAddress(address: string, field: string = 'address'): void {
  const ethereumAddressRegex = /^0x[a-fA-F0-9]{40}$/;
  
  if (!ethereumAddressRegex.test(address)) {
    throw new ValidationError(
      `${field} must be a valid Ethereum address (0x followed by 40 hexadecimal characters)`,
      field
    );
  }
}
...
export async function submitBet(params: BetParams): Promise<BetResult> {
  ...
  // Make API request
  const result = await request<BetResult>('submitBet', {
    method: 'POST',
    data: {
 ...
 */
export async function getBetHistory(address: string): Promise<Bet[]> {
  // Validate address format
  validateAddress(address, 'address');
  
  // Make API request with address as path parameter
  const bets = await request<Bet[]>('getBetHistory', {
    method: 'GET',
    pathParams: {
      address: address
    }
